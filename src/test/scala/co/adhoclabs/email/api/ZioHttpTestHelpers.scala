...
trait ZioHttpTestHelpers extends AsyncFunSpec with AsyncMockFactory with OneInstancePerTest {
  def provokeServerFailure(
    ...
    import co.adhoclabs.email.api.Schemas.errorResponseSchema

    val (status, errorResponse) =
      invokeZioRequest[ErrorResponse](app, request).left
  ...
  def provokeServerSuccess[T: Schema](
    ...
    val (status, errorResponse) =
      invokeZioRequest(app, request).right
        .getOrElse(throw new Exception("Broken successful test!"))
    assert(status == expectedStatus)
    assert(payloadAssertion(errorResponse))
  }

  private def invokeZioRequest[T: Schema](
      app: HttpApp[Any],
      request: Request
  ): Either[(Status, ErrorResponse), (Status, T)] = {
    import co.adhoclabs.email.api.Schemas.errorResponseSchema
    ...
    Unsafe.unsafe { implicit unsafe =>
      runtime.unsafe.run {
        (for {
          ...
          _        <-
            ZIO.when(response.status.isError)(
              for {
