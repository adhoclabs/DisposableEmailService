...
import co.adhoclabs.email.exceptions.{
  AlbumAlreadyExistsException,
  AlbumNotCreatedException,
  NoSongsInAlbumException
}
import co.adhoclabs.email.models.{Album, AlbumWithSongs, CreateAlbumRequest, PatchAlbumRequest}
  ...
  describe("PATCH /albums/:id") {
    ...
    it("should call AlbumManager.update and return updated song when it exists") {
      ...
      provokeServerSuccess[Album](
        app,
        Request.patch(
          s"albums/${expectedAlbumWithSongs.album.id}",
          body = Body.from(expectedAlbumWithSongs.album)
        ),
  ...
  describe("POST /albums") {
    ...
    it("should return a 400 response when the album has no songs") {
      ...
      provokeServerFailure(
        ...
        expectedStatus = Status.BadRequest,
        error =>
          error.error == s"Not creating album entitled ${createAlbumRequest.title} because it had no songs."
      )
    ...
    describe("DELETE /albums/:id") {
      it("should call AlbumManager.delete and return an empty 204") {
        ...
        import co.adhoclabs.email.api.Schemas.schema
