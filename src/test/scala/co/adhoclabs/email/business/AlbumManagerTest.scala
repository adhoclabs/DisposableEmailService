...
class AlbumManagerTest extends BusinessTestBase {
  ...
  describe("create") {
    ...
    it(
      "should throw an exception from AlbumDao.create when attempting to create an album with an ID that already exists"
    ) {
      (albumDao.create _)
        .expects(*)
  ...
  describe("patch") {
    val expectedUpdatedAlbum =
      expectedAlbumWithSongs.album.copy(
        title = "Updated Title",
        genre = Genre.HipHop
      )
    val patchRequest         =
