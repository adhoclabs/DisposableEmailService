...
import co.adhoclabs.email.exceptions.AlbumAlreadyExistsException
import co.adhoclabs.email.models.Genre._
import co.adhoclabs.email.models.{Album, AlbumWithSongs}
...
class AlbumDaoTest extends DataTestBase {
  ...
  override def withFixture(test: NoArgAsyncTest): FutureOutcome = {
    complete {
      ...
      val cleanupFutures =
        albumIdsToCleanUp.asScala.toList.map { case (albumId, _) =>
  ...
  describe("AlbumDao") {
    describe("create, get, update, delete") {
      it("should correctly execute the lifecycle of an album") {
        ...
        albumDao.create(AlbumWithSongs(expectedAlbumWithSongs.album, expectedAlbumWithSongs.songs)) flatMap {
          createdAlbumWithSongs: AlbumWithSongs =>
              ...
              case Some(actualAlbumWithSongs) =>
                ...
                albumDao.get(expectedAlbumWithSongs.album.id) flatMap {
                  case Some(actualAlbum) =>
                    ...
                    val expectedUpdatedAlbum =
                      expectedAlbumWithSongs.album.copy(title = "updated title", genre = Rock)
