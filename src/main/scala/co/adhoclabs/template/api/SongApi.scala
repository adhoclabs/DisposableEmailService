...
object SongApiEndpoints {
  ...
  val getSong =
    Endpoint(
      Method.GET / "songs" / (uuid("songId").??(openApiSrcLink(implicitly[sourcecode.Line])) ?? Doc.p(
  ...
  val updateSong =
    Endpoint(
      Method.PUT / "songs" / uuid("songId").??(openApiSrcLink(implicitly[sourcecode.Line])) ?? Doc.p(
        "The unique identifier of the song"
      )
    )
      .in[Song]
  ...
  val deleteSong =
    Endpoint(
      Method.DELETE / "songs" / uuid("songId").??(openApiSrcLink(implicitly[sourcecode.Line])) ?? Doc.p(
        "The unique identifier of the song"
      )
    )
...
case class SongRoutes(
  ...
  val getSong =
    SongApiEndpoints.getSong.implement(
      Handler.fromFunctionZIO { (songId: UUID) =>
        ZIO
          .fromFuture(implicit ec => songManager.get(songId))
          .mapError { case throwable: Throwable =>
  ...
  val createSong =
    SongApiEndpoints.createSong.implement(
      Handler.fromFunctionZIO { (createSongRequest: CreateSongRequest) =>
        ZIO
          .fromFuture(implicit ec => songManager.create(createSongRequest))
          .tapError(err => ZIO.debug("Err: " + err))
          .mapError {
  ...
  val updateSong =
    SongApiEndpoints.updateSong.implement(
      Handler.fromFunctionZIO { case (songId: UUID, song: Song) =>
        ZIO
          .fromFuture(implicit ec => songManager.update(song))
          .mapError { case throwable: Throwable =>
  ...
  val deleteSong =
    SongApiEndpoints.deleteSong.implement(
      Handler.fromFunctionZIO { (songId: UUID) =>
        ZIO.fromFuture(implicit ec => songManager.delete(songId)).mapError {
